...
export default function Home() {
  ...
  return (
    <main>
      <nav className="nav shell"><div className="brand"><span className="brand-mark">P</span><span>Pagaska <small>Help Center</small></span></div><a className="nav-link" href="#help">Bantuan</a></nav>
      <section className="hero shell"><div className="hero-copy"><div className="eyebrow"><Sparkles size={15}/> Pusat bantuan resmi Pagaska</div><h1>Kalau Pagaska bermasalah,<br/><span>kita cari tahu bareng.</span></h1><p>Jangan panik kalau APK tidak bisa dipasang. Pilih masalahmu dan ikuti langkah yang paling mudah dulu.</p><div className="hero-actions"><a className="primary" href="https://github.com/Vexcompany/Pagaska-Music/releases">Download Pagaska <ArrowRight size={17}/></a><a className="secondary" href="#help"><HelpCircle size={17}/> Saya butuh bantuan</a></div></div><div className="helper-card"><div className="avatar">🌸</div><div><div className="helper-name">Ryuna <span>online</span></div><p>“Hai! Ceritakan apa yang terjadi. Kita cek pelan-pelan, tanpa istilah ribet.”</p></div></div></section>
...
function Chat({ onClose }) {
  const [messages, setMessages] = useState([{ role: 'assistant', content: 'Hai! Aku Ryuna 🌸 Apa yang terjadi saat kamu mencoba memakai Pagaska?' }]);
  ...
  return <div className="chat-backdrop" onMouseDown={(e) => e.target === e.currentTarget && onClose()}><section className="chat"><header><div><strong>🌸 Ryuna</strong><span>Pagaska Help Assistant</span></div><button onClick={onClose} aria-label="Tutup"><X/></button></header><div className="chat-body">{messages.map((m, i) => <div key={i} className={`bubble ${m.role}`}>{m.content}</div>)}{busy && <div className="bubble assistant">Ryuna sedang berpikir…</div>}</div><form onSubmit={send}><input value={input} onChange={(e) => setInput(e.target.value)} placeholder="Contoh: Pagaska tidak bisa di-install"/><button className="primary" disabled={busy}>Kirim</button></form></section></div>;
}
